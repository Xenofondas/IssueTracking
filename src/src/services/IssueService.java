...
public class IssueService {
	...
	public void createIssue(Issue issue) throws SQLException {
		...
		String query = "INSERT INTO ist_issues (title, description, category, time_modified, modified_by, created_by, status,"
				+ "assigned_to, time_created ) VALUES ('" + issue.getTitle() + "'," + " '" + issue.getDescription()
				+ "', '" + issue.getCategory() + "', '" + issue.getTimeModified() + "'," + " '" + issue.getModifiedBy()
				+ "', '" + issue.getCreatedBy() + "', '" + issue.getStatus() + "'," + " '" + issue.getAssignedTo()
				+ "', '" + issue.getTimeCreated() + "')";
	...
	public List<Issue> getAllIssues() throws SQLException {
		...
		while (rs.next()) {
			issues.add(new Issue(rs.getLong("id"), rs.getString("title"), rs.getString("description"),
					rs.getString("category"), rs.getString("time_created"), rs.getString("time_modified"),
					rs.getString("created_by"), rs.getString("status"), rs.getString("assigned_to"),
	...
	public Issue findIssueById(Long id) throws SQLException {
		...
		Issue issue = new Issue(rs.getLong("id"), rs.getString("title"), rs.getString("description"),
				rs.getString("category"), rs.getString("time_created"), rs.getString("time_modified"),
				rs.getString("created_by"), rs.getString("status"), rs.getString("assigned_to"),
				rs.getString("modified_by"));
	...
	public void updateIssue(Issue issue) throws SQLException {
		...
		String query = "UPDATE ist_issues  SET title='" + issue.getTitle() + "', description='" + issue.getDescription()
				+ "', category='" + issue.getCategory() + "'," + "time_modified='" + issue.getTimeModified()
				+ "', modified_by='" + issue.getModifiedBy() + "', status='" + issue.getStatus() + "',"
